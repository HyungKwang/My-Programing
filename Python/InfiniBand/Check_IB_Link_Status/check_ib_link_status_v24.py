...
command5 = "  #python check_ib_link_status_v15.py -i ./tem/ibdiagnet2 -a -n 20 -s -o C:\Python39\study\save_backdata.xlsx\n"
...
if not df_pm.empty:
    ...
    if "--extended_speeds" in running_command_only :
        ...
        if not df_fec_uncorrectable.empty:
            print(
                f"\n\nPort FEC Uncorrectable Counters (Qt :{len(df_max_retrans.index)}) ",
                f"\n##################################################",
...
if not df_pm.empty:
    ...
    if ibdiagnet_cable_file_exist : 
        ...
        df_cable_3rd_party = df_cable_all[df_cable_all["Vendor"] != "Mellanox"].copy()
        ...
        if not df_cable_3rd_party.empty:
            print(
                f"\n\n3rd Party used  (Qt : {len(df_cable_3rd_party.index)})",
                f"\n##################################################",
                f"\n{df_cable_3rd_party.head(args.top_n).to_string(index=False)}",
            )
        else:
            print(
                f"\n\nno 3rd Party used Cable found",
                f"\n##################################################",
            )
        

        df_cable_temperature = df_cable_all[df_cable_all["Temperature"] > 70].copy()        
        ...
        if not df_cable_temperature.empty:
            ...
            print(
                ...
                f"\n##################################################",
                f"\n\nCable Temperature: < 70c on all links.",
            )
