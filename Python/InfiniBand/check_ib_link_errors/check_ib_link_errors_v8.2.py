  ...
  #  df_pm.to_csv("before") 
   ...
   # df_pm["CongestionIndexExt"] = df_pm["CongestionIndexExt"].astype({'CongestionIndexExt':'int'})
    ...
    if "--pm_pause_time" in running_command_only :
            ...
            df_pm["Lost_Bandwidth(%)"] = ( 
                df_pm["PortXmitWaitExt"] * 64 / (pm_pause_time_value * 1024 * 1024 * 1024)
            )

            #df_pm["Lost_Bandwidth"] = df_pm["Lost_Bandwidth"].apply(int, base=16)
    
            #except ZeroDivisionError:
              # df_pm["CongestionIndexExt"] = 0

            #df_pm["Lost_Bandwidth"] = df_pm["Lost_Bandwidth"].round(0)
           # df_pm["Lost_Bandwidth"] = df_pm["Lost_Bandwidth"].astype({'Lost_Bandwidth':'int64'})
           # df_pm["Lost_Bandwidth"] = (    
           #           df_pm["Lost_Bandwidth"] / 1024 * 1024 * 1024
            
            #)
            df_pm["Lost_Bandwidth(Gbps)"] = df_pm["Lost_Bandwidth(Gbps)"].round(2)
            df_pm["Lost_Bandwidth(%)"] = df_pm["Lost_Bandwidth(%)"].round(2)

            df_Lost_Bandwidth = df_pm[df_pm["Lost_Bandwidth(Gbps)"] > 0]
            df_Lost_Bandwidth = df_pm[df_pm["Lost_Bandwidth(%)"] > 0]
