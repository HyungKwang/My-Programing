...
parser.add_argument(
    ...
    help="Display TX/RX Bandwiths in details ",
)
parser.add_argument(
...
if not df_pm.empty:
    ...
    ###Recevie & Transmit Bandwidth
        ...
        #5.9 (Tier1 & Tier4) Servers -> Switch By TX Bandwidth (Gbps)
            ...
            df_Server_Tx_Bandwidth = df_Server_Tx_Bandwidth.sort_values(by="Lost_Bandwidth(Gbps)", ascending=False)
        ...
        #5.11 (Tier2 & Tier3) Switch <-> Switch By TX Bandwidth (Gbps)
         ...
         ### "5.7 Lost BandWidth" logic implemented here
            ...
            df_Lost_Bandwidth["PortXmitDataExtended(Gbps)"] = df_Lost_Bandwidth["PortXmitDataExtended(Gbps)"].astype(str)
            df_Lost_Bandwidth["PortRcvDataExtended(Gbps)"] = df_Lost_Bandwidth["PortRcvDataExtended(Gbps)"].astype(str)
    ...
    if "--pm_pause_time" in running_command_only :
        ...
        if args.all:
            ...
            if not df_Server_Tx_Bandwidth.empty:
                ...
                print(
                    f"\n Sorted by PortXmitDataExtended ",
                    f"\n------------------------------------------",
                    f"\n{df_Server_Tx_Bandwidth.head(args.top_n).to_string(index=False)}",
                )
