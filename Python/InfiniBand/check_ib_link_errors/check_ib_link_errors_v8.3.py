...
# v8.3: 6/14/2022  modified by HyungKwang Choi & Dan Fixed "unsupported extended counters (in older FDR device and old SX6710 GW device)"
#==================================================================================
...
if not df_pm.empty:
    ...
    if "--extended_speeds all" in running_command_only :   # extended speed 없을때 PortRcvDataExtended 이렇게 값이 저장되는지 확인해보기
        ...
        pm_cols = [
            ...
            "ExcessiveBufferOverrunErrors",
            "max_retransmission_rate",
            "PortRcvDataExtended",   # extended speed 없을때 PortRcvDataExtended 이렇게 값이 저장되는지 확인해보기
            "PortXmitDataExtended",
            "LWA",
            "LSA",
        ]
    ...
    df_pm["PortXmitWait"] = df_pm["PortXmitWait"].apply(lambda x : format(x,"#018x"))
 
    df_pm["PortXmitWait"] = df_pm["PortXmitWait"].astype('object')
   ...
   # df_pm["CongestionIndexExt"] = df_pm["CongestionIndexExt"].astype({'CongestionIndexExt':'int'})
    ...
    # 4.8 Congestion BW percentage.
            ...
            df_Lost_Bandwidth = df_pm[df_pm["Lost_Bandwidth(Gbps)"] > 0]
            ...
            ["SrcDevice", "SrcPort", "SrcGUID", "PortXmitWaitExt", "Lost_Bandwidth(Gbps)", "DstDevice", "DstGUID", "DstPort"]
            ]
    ...
    # 4.8 Recevie & Transmit Bandwidth
        ...
        #Tier1 & Tier4 Switch <-> Servers By RX Bandwidth (Gbps)
            ...
            ["DstDevice", "DstPort", "DstGUID", "PortRcvDataExtended(Gbps)", "SrcDevice", "SrcPort", "SrcGUID"]
            ]      
            ...
            df_Server_Rx_Bandwidth = df_Server_Rx_Bandwidth.sort_values(by="PortRcvDataExtended(Gbps)", ascending=False)
       
   
        #Tier1 & Tier4 Switch <-> Servers By TX Bandwidth (Gbps)
            df_Server_Tx_Bandwidth = df_Server_Tx_Bandwidth[
            ["SrcDevice", "SrcPort", "SrcGUID", "PortXmitDataExtended(Gbps)", "DstDevice", "DstPort", "DstGUID" ]
            ]      
            ...
            df_Server_Tx_Bandwidth = df_Server_Tx_Bandwidth.sort_values(by="PortXmitDataExtended(Gbps)", ascending=False)

 

        #Tier2 & Tier3 Switch <-> Switch By RX Bandwidth (Gbps)
            df_Switch_Rx_Bandwidth = df_Switch_Rx_Bandwidth[
            [ "SrcDevice", "SrcPort", "SrcGUID","PortRcvDataExtended(Gbps)", "DstDevice", "DstPort", "DstGUID",]
            ]      
            ...
            df_Switch_Rx_Bandwidth = df_Switch_Rx_Bandwidth.sort_values(by="PortRcvDataExtended(Gbps)", ascending=False)
        ...
        #Tier2 & Tier3 Switch <-> Switch By TX Bandwidth (Gbps)
            ...
            df_Switch_Tx_Bandwidth = df_Switch_Tx_Bandwidth.sort_values(by="PortXmitDataExtended(Gbps)", ascending=False)
       

    # 4.9 Congested Recevie & Transmit Bandwidth (%)
...
if args.output_file:
    ...
    if not df_pm.empty:
        ...
        if "--pm_pause_time" in running_command_only :
            ...
            if not df_Server_Rx_Bandwidth.empty:
                df_Server_Rx_Bandwidth.to_excel(writer, sheet_name="Lost_Bandwidth", index=False)

            if not df_Tt_Bandwidth.empty:
                df_Tt_Bandwidth.to_excel(writer, sheet_name="Lost_Bandwidth", index=False)
