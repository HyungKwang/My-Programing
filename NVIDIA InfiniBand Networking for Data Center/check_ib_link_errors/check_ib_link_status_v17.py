...
parser.add_argument(
    "-s", "--lid_checking", 
    type=str,         
    help="lid_checking : Skip Invalid LID & Mcast LID checking ",
)
...
command5 = "  #python check_ib_link_status_v15.py -i ./tem/ibdiagnet2 -a -n 20 -s lid_checking -o C:\Python39\study\save_backdata.xlsx\n"
...
if args.lid_checking == "lid_checking" :
...
if temp_invalid_checking :
        ...
        if len(warn_return) >0  :
                print(
                    f"\n\nInvalid LID number or Mcast LID found (to skip this, please return with -s lid_checking)",
                    f"\n##############################################################################################",
                )      
