# =============================================================================
# File name: check_ib_link_status_v45.py
...
# v37 : 3/25/2026  modified by HyungKwang  added 2 things
#                  1). 'debug' feature which outputs all row data to csv file for the debug/troubleshoot purpose.
# v38 : 4/22/2026  modified by HyungKwang  added 1 things
#                  1. added missing 'head(args.top_n)' to PSU part.
# v39: 5/6/2026  modified by HyungKwang  print GPU Inventory Summary when NODES has descriptions matching "GB* Nvidia Technologies" 
#                As of v39, my script will also check GPU related things..
# v40: 5/27/2026  modified by HyungKwang  Adding new feature checking 'Rail Optimized Topology Validation'
# v41: 5/27/2026  modified by HyungKwang  Removing unnecessary print statements.
# v42: 5/27/2026  modified by HyungKwang  Check ibdiagnet2.ibnetdiscover for ports UP but remote peer not responding
# v43: 7/2/2026  modified by HyungKwang  Added IB device models info in the code
# v44: 7/7/2026  modified by HyungKwang  Added PCI degradation warning validation
# v45: 7/7/2026  modified by HyungKwang  Added -n limits and affected-item counts for Rail Optimized Topology and PCI degradation warnings
#==================================================================================
#
# 
# 
...
###### Below is newly created since the script verion 37.Sometimes, whatever reason, ibdiagnet collects weird string which causes error when writing data to xlsx file.
###### #openpyxl / Excel XML cannot store NUL and most C0 control characters; strip them before to_excel.
_ILLEGAL_XML_1_0_TEXT_CHARS = re.compile(r"[\x00-\x08\x0B\x0C\x0E-\x1F]")


def sanitize_str_for_excel(val):
    if pd.isna(val) or not isinstance(val, str):
        return val
    s = _ILLEGAL_XML_1_0_TEXT_CHARS.sub("", val)
    s = s.replace("\ufffd", "")
    return s.strip()


def normalize_vendor(v):
    """Vendor from ibdiagnet may include hidden bytes; normalize before comparisons and Excel."""
    if pd.isna(v):
        return ""
    s = sanitize_str_for_excel(str(v))
...
def sanitize_object_columns(df):
    ...
    for col in out.select_dtypes(include=["object"]).columns:
        out[col] = out[col].map(
            lambda x: sanitize_str_for_excel(x) if isinstance(x, str) else x
        )
    return out


def extract_warning_block(log_lines, warning_marker):
    """Return warning lines from ``warning_marker`` up to the next info line."""
    warning_lines = []
    marker_found = False
    ...
    for raw_line in log_lines:
        ...
        if not marker_found:
            if stripped_line == warning_marker:
                marker_found = True
            else:
                continue
        ...
        if stripped_line.startswith("-W-"):
            warning_lines.append(stripped_line[3:].lstrip())

    return warning_lines


def extract_rail_topology_warnings(log_lines):
    """Return the top-level affected items from Rail Optimized Topology validation."""
    warning_lines = []
    in_rail_section = False
    summary_line = ""

    for raw_line in log_lines:
        stripped_line = raw_line.strip()

        if not in_rail_section:
            if stripped_line == "Rail Optimized Topology Validation":
                in_rail_section = True
            continue

        if "Rail Optimized Topology Validation ended with" in stripped_line:
            summary_line = stripped_line[3:].lstrip()
            break

        if stripped_line.startswith("-W-"):
            warning_line = stripped_line[3:].strip()
            if (
                warning_line != "Retrieve Rails Data finished with warnings"
                and not warning_line.lower().startswith("rail ")
            ):
                warning_lines.append(warning_line)

    return summary_line, warning_lines


def unique_output_path(path):
    """If *path* already exists, return path with a numeric suffix (name_1.ext, name_2.ext, ...)."""
...
PSID_Mapping_table = {
    "MT_0000000577": "MT_0000000577 => QM9700 (NVIDIA_Managed_switch,NDR)",
    "MT_0000000579": "MT_0000000579 => QM9790 (NVIDIA_Unmanaged_switch,NDR)",
    "MT_0000000836": "MT_0000000836 => QM9700 (NVIDIA_Managed_switch,NDR)",
    "MT_0000000837": "MT_0000000837 => QM9790 (NVIDIA_Unmanaged_switch,NDR)",
    "HPE0000000065": "HPE0000000065 => HPE, NDR Switch",
    "MT_0000001026": "MT_0000001026 => Q3200 (NVIDIA_Managed_switch,XDR)",
    "MT_0000001027": "MT_0000001027 => Q3400 (NVIDIA_Managed_switch,XDR)",
    "MT_0000001053": "MT_0000001053 => Q3400 (NVIDIA_Managed_switch,XDR)",
    "MT_0000001442": "MT_0000001442 => Q3401 (NVIDIA_Managed_switch,XDR)",
    "MT_0000000062": "MT_0000000062 => QM8700 (NVIDIA_Managed_switch,HDR)",
    "MT_0000000063": "MT_0000000063 => QM8790 (NVIDIA_Unmanaged_switch,HDR)",
    "LNV0000000020": "LNV0000000020 => QM8790 (Lenovo_Unmanaged_switch,HDR)",
    "LNV0000000021": "LNV0000000021 => QM8790 (Lenovo_Unmanaged_switch,HDR)",
    "NVD0000000050": "NVD0000000050 => B200/B300_HGX_Quantum-3_NVSwitch",
    "MT_0000001330": "MT_0000001330 => NVLink Switch Tray",
    "LNV0000000052": "LNV0000000052 => QM9700 (Lenovo_Managed_switch,NDR)",
    "LNV0000000053": "LNV0000000053 => QM9790 (Lenovo_Unmanaged_switch,NDR)",
    "DEL0000000036": "DEL0000000036 => CX-7 (DELL, NDR 1-Port OSFP IB Adapter Card)",
    "DEL0000000051": "DEL0000000051 => CX-7 (DELL, NDR 1-Port VPI OSFP Adapter)",
        
    "MT_0000000224": "MT_0000000224 => CX-6 (NVIDIA, 100Gb/s(HDR100, EDR IB, 100GbE))",
    "MT_0000000225": "MT_0000000225 => CX-6 (NVIDIA, HDR IB (200Gb/s) and 200GbE; dual-port QSFP56)",
    "MT_0000000892": "MT_0000000892 => CX-7 (NVIDIA, NDR, dual-port QSFP)",
        
    "MT_0000000691": "MT_0000000691 => CX-6 (NVIDIA, HDR dual-port QSFP56)",
    "MT_0000000838": "MT_0000000838 => CX-7 (NVIDIA, NDR IB 1-port OSFP)",
    "MT_0000001121": "MT_0000001121 => CX-7 (NVIDIA, NDR)",
    "MT_0000000891": "MT_0000000891 => CX-7 (NVIDIA, NDR VPI 2ports)",
    "MT_0000000970": "MT_0000000970 => CX-7 (HPE, NDR InfiniBand 1-port OSFP)",
    "MT_0000000971": "MT_0000000971 => CX-7 (HPE, NDR-200 InfiniBand 1-port OSFP)",
    "LNV0000000019": "LNV0000000019 => CX-6 (Lenovo, HDR/200GbE QSFP56 2-port VPI Adapter)",
    "LNV0000000058": "LNV0000000058 => CX-7 (Lenovo, NDR200/HDR QSFP112 2-port )",
    "MT_0000000451": "MT_0000000451 => CX-6 (HPE InfiniBand HDR/Ethernet 200Gb 1-port MCX653105A-HDAT)",
    "MT_0000001058": "MT_0000001058 => CX-7 (ConnectX-7 OCP3.0 TSFF Adapter Card; 400GbE / NDR IB (default mode); Single-port OSFP)",
}
...
parser.add_argument(
    ...
    help="Collecting all row data to a csv format file for debugging purpose",
)
...
command1 = "\n\nExamples (How to run) > : \n\n  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 \n"
command2 = "  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 -a \n"
command3 = "  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 -a -n 20 \n"
command4 = "  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 -a -n 20 -o  C:\\Python39\\study\\save_backdata.xlsx \n"
command5 = "  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 -a -n 20 -s -o C:\\Python39\\study\\save_backdata.xlsx \n"
command6 = "  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 -a -n 20 -c -s -o C:\\Python39\\study\\save_backdata.xlsx \n"
command7 = "  #python check_ib_link_status_v45.py -i ./tem/ibdiagnet2 -d"
parser.usage = usage.rstrip() + command1 + command2 + command3 + command4 + command5 + command6 + command7
...
Aport_found = False
Port_Speed_Width_found = False
Rail_Optimized_found = False
remote_peer_not_responding = []

_REMOTE_PEER_WARNING = "Warning: the port is UP, but remote peer is not responding"
_SWITCHGUID_PREFIX = "switchguid="


###########################################################################################################################################################
########################## Remote peer not responding (ibdiagnet2.ibnetdiscover) ########################################################################

def find_remote_peer_not_responding(ibnetdiscover_lines):
    """Return list of (switchguid line, port line) for non-responding remote peers."""
    findings = []
    current_switchguid = None

    for raw_line in ibnetdiscover_lines:
        line = raw_line.rstrip("\n\r")
        if line.startswith(_SWITCHGUID_PREFIX):
            current_switchguid = line.strip()
        elif _REMOTE_PEER_WARNING in line and current_switchguid:
            findings.append((current_switchguid, line.strip()))

    return findings


def group_remote_peer_findings(findings):
    """Group port lines by switchguid, preserving order and removing duplicate lines."""
    grouped = []
    seen_switchguids = set()
    for switchguid, port_line in findings:
        if switchguid not in seen_switchguids:
            seen_switchguids.add(switchguid)
            grouped.append((switchguid, []))
        grouped[-1][1].append(port_line)

    result = []
    for switchguid, port_lines in grouped:
        unique_lines = list(dict.fromkeys(port_lines))
        result.append((switchguid, unique_lines))
    return result
...
"""
    # Step 1 Print checking the running parameter
"""
if args.top_n == 10:
    print(
...
"""
    # Step 2 Print the running commands
"""
...
if os.path.isfile(ibdgnt_log):
    with open(ibdgnt_log, "r", encoding="utf-8", errors="ignore") as log_file:
        ...
        #Aport checking
        Aport_list = []
        _aport_sym_marker = "APort Symmetry Check finished with errors"
        Aport_found = False
        for _log_line in log_file:
            if _aport_sym_marker in _log_line:
                Aport_found = True
                for _log_line2 in log_file:
                    _s2 = _log_line2.strip()
                    if not _s2:
                        continue
                    if _s2.startswith("-I-"):
                        break
                    line = _log_line2.rstrip("\n\r")
                    line = line.replace("-E-", "")   # 여기에서 '-E-' 제거
                    Aport_list.append(line)
                break

        #Aport Speed / Width checks
        Port_Speed_Width_list = []
        _port_Speed_Width_sym_marker = "Links Speed Check finished with errors"
        Port_Speed_Width_found = False
        log_file.seek(0)
        for _log_line in log_file:
            if _port_Speed_Width_sym_marker in _log_line:
                Port_Speed_Width_found = True
                for _log_line2 in log_file:
                    _s2 = _log_line2.strip()
                    if not _s2:
                        continue
                    ...
                    line = _log_line2.rstrip("\n\r")
                    line = line.replace("-E-", "")   # 여기에서 '-E-' 제거
                    Port_Speed_Width_list.append(line)
            
                break

        # Rail Optimized Topology Validation
        log_file.seek(0)
        Rail_Optimized_line, Rail_Optimized_list = (
            extract_rail_topology_warnings(log_file)
        )
        Rail_Optimized_found = bool(Rail_Optimized_line)

        # PCI degradation validation
        _pci_degradation_marker = "-W- PCI degradation check finished with warnings"
        log_file.seek(0)
        PCI_degradation_block = extract_warning_block(
            log_file, _pci_degradation_marker
        )
        PCI_degradation_list = PCI_degradation_block[1:]
...
ibdgnt_discover = os.path.join(args.ibdiagnet_folder, "ibdiagnet2.ibnetdiscover")
ibnetdiscover_file_exist = os.path.isfile(ibdgnt_discover)

if ibnetdiscover_file_exist:
    ...
    remote_peer_not_responding = find_remote_peer_not_responding(discover_dump)

else:
    print(
        "\nERROR: Couldn't find ibdiagnet2.ibnetdiscover file. "
        "Skipping ibnetdiscover checks ..."
    )
    if temp_invalid_checking:
        temp_invalid_checking = False
...
df_gpu_ib_nodes = pd.DataFrame()
df_gpu_inventory_tbl = pd.DataFrame()
...
if os.path.isfile(ibdgnt_db):
    ...
    df_gpu_FW_info_tbl = pd.DataFrame()
    ...
    # NODES rows for NVIDIA GPU IB devices, e.g. "GB200 Nvidia Technologies"
    if not df_nodes_tbl.empty:
        _node_desc_nv = df_nodes_tbl["NodeDesc"].map(normalize_vendor)
        _gpu_node_mask = _node_desc_nv.str.match(
            r"^GB\S+\s+Nvidia Technologies$", na=False
        )
        if _gpu_node_mask.any():
            df_gpu_ib_nodes = df_nodes_tbl.loc[_gpu_node_mask].copy()
            df_gpu_ib_nodes["GPUName"] = _node_desc_nv.loc[_gpu_node_mask].str.extract(
                r"^(GB\S+)\s+Nvidia Technologies$", expand=False
            ).values
            # Only parse GENERAL_INFO_SMP when GPU nodes exist
            df_gpu_FW_info_tbl = db_csv_extr("GENERAL_INFO_SMP", db_csv)

    # GPU FW info from GENERAL_INFO_SMP (match NodeGUID -> join with NODES)
    df_gpu_fw_ver_tbl = pd.DataFrame()
    if not df_gpu_FW_info_tbl.empty:
        df_gpu_FW_info_tbl = sanitize_object_columns(df_gpu_FW_info_tbl)
        if "NodeGuid" in df_gpu_FW_info_tbl.columns and "NodeGUID" not in df_gpu_FW_info_tbl.columns:
            df_gpu_FW_info_tbl = df_gpu_FW_info_tbl.rename(columns={"NodeGuid": "NodeGUID"})

        # Create FWInfo_Version from FWInfo_Extended_* hex fields (same logic as NODES_INFO)
        _gpu_fw_ver_cols = {
            "FWInfo_Extended_Major",
            "FWInfo_Extended_Minor",
            "FWInfo_Extended_SubMinor",
        }
        if "NodeGUID" in df_gpu_FW_info_tbl.columns and _gpu_fw_ver_cols.issubset(
            set(df_gpu_FW_info_tbl.columns)
        ):
            df_gpu_FW_info_tbl["FWInfo_Version"] = (
                df_gpu_FW_info_tbl["FWInfo_Extended_Major"].apply(int, base=16).astype(str)
                + "."
                + df_gpu_FW_info_tbl["FWInfo_Extended_Minor"].apply(int, base=16).astype(str)
                + "."
                + df_gpu_FW_info_tbl["FWInfo_Extended_SubMinor"].apply(int, base=16).astype(str)
            )
            df_gpu_fw_ver_tbl = df_gpu_FW_info_tbl[["NodeGUID", "FWInfo_Version"]].copy()
            df_gpu_fw_ver_tbl = df_gpu_fw_ver_tbl.rename(columns={"FWInfo_Version": "GPU_FW_Version"})
            df_gpu_fw_ver_tbl["GPU_FW_Version"] = df_gpu_fw_ver_tbl["GPU_FW_Version"].astype(str).str.strip()
            df_gpu_fw_ver_tbl = df_gpu_fw_ver_tbl[df_gpu_fw_ver_tbl["GPU_FW_Version"].ne("")]

    if not df_gpu_ib_nodes.empty:
        if not df_gpu_fw_ver_tbl.empty:
            df_gpu_inventory_tbl = pd.merge(
                left=df_gpu_ib_nodes[["NodeGUID", "GPUName"]],
                right=df_gpu_fw_ver_tbl,
                on="NodeGUID",
                how="left",
            )
            df_gpu_inventory_tbl["GPU_FW_Version"] = df_gpu_inventory_tbl["GPU_FW_Version"].fillna("Unknown")
        else:
            df_gpu_inventory_tbl = df_gpu_ib_nodes[["NodeGUID", "GPUName"]].copy()
            df_gpu_inventory_tbl["GPU_FW_Version"] = "Unknown"
...
if not df_nodes_info_tbl.empty and not df_sw_tbl.empty and not df_sw_info_tbl.empty: 
    ...
    # If NodeDesc has no space, rsplit returns one column only — reindex to two columns.
        _hca_desc_split = df_hca_tbl["NodeDesc"].str.rsplit(" ", n=1, expand=True).reindex(
        columns=[0, 1]
    )
    df_hca_tbl[["HostName", "PortNum"]] = _hca_desc_split
...
############# Remove NUL / illegal XML chars from cable fields; normalize Vendor so "NVIDIA" variants filter & Excel work####
############# That means, below function remove weird string in ibdiagnet2.cables before saving it to XLSX file.
        df_cable_all = sanitize_object_columns(df_cable_all)
        df_cable_all["Vendor"] = df_cable_all["Vendor"].map(normalize_vendor)
##################################################################################
    ...
    if args.debug:
    	df_pm.to_csv("For_debugging_data_all")      
...
"""
    # Step 3 Print IB Inventory lists (Switch & HCA FW info and quantity)
"""

#  Print Switch inventory summary
if not df_nodes_info_tbl.empty and not df_sw_tbl.empty and not df_sw_info_tbl.empty : 
    ...
    for i in range(df_ib_sw_pivot.index.size):
        print("--------------------------------------------------")
        _psid_sw = df_ib_sw_pivot.index[i]
        print(f"PSID: {PSID_Mapping_table.get(_psid_sw, _psid_sw)}")
        print("--------------------------------------------------")
...
#  Print HCA inventory summary
if not df_nodes_info_tbl.empty and not df_sw_tbl.empty and not df_sw_info_tbl.empty : 
    print(f"\n\nHCA Inventory Summary:")
    print(f"Total HCAs : {len(df_hca_tbl)}")
    print(f"##################################################")
    for i in range(df_hca_pivot.index.size):
        print("--------------------------------------------------")
        _psid_hca = df_hca_pivot.index[i]
        print(f"PSID: {PSID_Mapping_table.get(_psid_hca, _psid_hca)}")
        print("--------------------------------------------------")
...
#  Print GPU inventory summary
if not df_gpu_inventory_tbl.empty:
    print(f"\n\nGPU Inventory Summary:")
    print(f"Total GPUs: {df_gpu_inventory_tbl['NodeGUID'].nunique()}")
    print(f"##################################################")
    for _gpu_name in sorted(df_gpu_inventory_tbl["GPUName"].dropna().unique()):
        print("--------------------------------------------------")
        print(f"GPU name : {_gpu_name}")
        print("--------------------------------------------------")
        _fw_qty = (
            df_gpu_inventory_tbl.loc[df_gpu_inventory_tbl["GPUName"] == _gpu_name, "GPU_FW_Version"]
            .fillna("Unknown")
            .astype(str)
            .value_counts()
            .sort_index()
        )
        for _fw_ver, _qty in _fw_qty.items():
            print(f"FW Ver.: {_fw_ver:>16}       Qty : {_qty:>4d}")
        print("--------------------------------------------------")
    print("")
...
"""
    # Step 4 checking all defcetives (Link flap, BER....)
"""
...
if ibdiagnet_cable_file_exist : 
    ...
    df_cable_3rd_party = df_cable_all[
        ~df_cable_all["Vendor"].str.upper().isin(["MELLANOX", "NVIDIA"])
    ]
...
if not df_power_supplies_tbl.empty:
    ...
    if not df_power_supplies_tbl.empty:
        print(
            ...
            f"\n##################################################",
            f"\n{df_power_supplies_tbl.head(args.top_n).to_string(index=False)}",
            )
                
    
###################################################################################################################################################
########################## This is to check other link related problem ############################################################################

if Aport_found:
        print(
            f"\n\n APort Symmetry Check finished with errors (Qt : {len(Aport_list) } ) ",
            f"\n##################################################",
            )

        for _aport_line in Aport_list[: max(0, args.top_n)]:
            print(_aport_line)
            
if Port_Speed_Width_found:
        print(
            f"\n\n Links Speed Check finished with errors (Qt : {len(Port_Speed_Width_list) } ) ",
            f"\n##################################################",
            )

        for _port_speed_width_line in Port_Speed_Width_list[: max(0, args.top_n)]:
            print(_port_speed_width_line)

if Rail_Optimized_found:
        print(
            f"\n\n{Rail_Optimized_line} (Qt : {len(Rail_Optimized_list)})",
            f"\n##################################################",
            )

        for _rail_optimized_warning in Rail_Optimized_list[: max(0, args.top_n)]:
            print(_rail_optimized_warning)

        if len(Rail_Optimized_list) > max(0, args.top_n):
            print(
                "\n ==> Please check the file 'ibdiagnet2.rail' for more details  <==="
            )

if PCI_degradation_list:
        print(
            f"\n\nPCI degradation warnings (Qt : {len(PCI_degradation_list)})",
            f"\n##################################################",
            )

        for _pci_degradation_line in PCI_degradation_list[: max(0, args.top_n)]:
            print(_pci_degradation_line)

_remote_peer_grouped = group_remote_peer_findings(remote_peer_not_responding)
_remote_peer_grouped_show = group_remote_peer_findings(
    remote_peer_not_responding[: max(0, args.top_n)]
)
if _remote_peer_grouped:
    print(
        f"\n\nPort UP but remote peer not responding "
        f"(Qt : {len(_remote_peer_grouped)} switch(es), "
        f"{len(remote_peer_not_responding)} port line(s))",
        f"\n##################################################",
    )
    for _switchguid, _port_lines in _remote_peer_grouped_show:
        print(_switchguid)
        for _port_line in _port_lines:
            print(_port_line)
...
if args.output_file:
    ...
    output_excel_path = unique_output_path(args.output_file)
    ...
    if output_excel_path != os.path.normpath(args.output_file):
        print(
            f"NOTE: Requested file already exists; using a new name:\n  {output_excel_path}\n",
        )
    ...
    # 8.1 Writing IB inventory lists (Switch, HCA, GPU)
    ...
    if not df_gpu_inventory_tbl.empty:
        df_gpu_pivot = pd.pivot_table(
            df_gpu_inventory_tbl,
            index="GPUName",
            columns="GPU_FW_Version",
            aggfunc="size",
        )
        df_gpu_pivot.to_excel(writer, sheet_name="GPU_Inventory_Summary")
    ...
    if Aport_list:
        sanitize_object_columns(pd.DataFrame({"log_line": Aport_list})).to_excel(
            writer, sheet_name="APort_Symmetry_Errors", index=False, header=False )

    if Port_Speed_Width_list:
        sanitize_object_columns(pd.DataFrame({"log_line": Port_Speed_Width_list})).to_excel(
            writer, sheet_name="Links_Speed_Width_Errors", index=False, header=False )

    if PCI_degradation_list:
        sanitize_object_columns(pd.DataFrame({"log_line": PCI_degradation_list})).to_excel(
            writer, sheet_name="PCI_degradation_warnings", index=False, header=False )

    if Rail_Optimized_list:
        sanitize_object_columns(pd.DataFrame({"log_line": Rail_Optimized_list})).to_excel(
            writer, sheet_name="Rail_topology_warnings", index=False, header=False )

    if _remote_peer_grouped:
        _remote_peer_rows = [
            {"switchguid": sg, "port_line": pl}
            for sg, port_lines in _remote_peer_grouped
            for pl in port_lines
        ]
        sanitize_object_columns(pd.DataFrame(_remote_peer_rows)).to_excel(
            writer, sheet_name="Remote_peer_not_responding", index=False
        )

    writer.close()
    print(f"Data is written successfully to Excel File: {output_excel_path}")
